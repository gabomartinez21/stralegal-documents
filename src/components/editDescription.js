...
function EditDescription() {
    ...
    const getInfo = async () =>{
        const descripcionDB = await axios.get(`${URLSERVER}/admin/v1/documentos.php?descripcion=${id}`);
        console.log(descripcionDB)
        if(descripcionDB.data.ok){
            setDocumento(descripcionDB.data.body[0].id_doc);
            setPrecio(descripcionDB.data.body[0].precio)
            setDescripcion(descripcionDB.data.body[0].descripcion_doc);
            setCategoria(descripcionDB.data.body[0].categoria)
        }
    }
    ...
    return (
        <Container>
            <div>
                <h1>Editar descripcion para documento</h1>
            </div>
            <Box
                component="form"
                noValidate
